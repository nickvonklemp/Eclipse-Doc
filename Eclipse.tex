...
\textbf{Eclipse} is fully configured on the UCT campus computers but there are some steps that you need to remember before getting started. First plug in your STM32 board, Windows should start searching for a pre-configured driver once it's plugged in.\includegraphics[height=3ex]{driverico}\\
\begin{figure}[htbp]
\centering
...
So download and install the \textbf{\color{Brown} GCC-ARM-NONE-EABI toolset} from \href{https://vula.uct.ac.za/x/m9Z91P}{\color{Blue} \underline{\smash{Vula}}}.Otherwise get it from the  \href{https://launchpad.net/gcc-arm-embedded}{\color{Blue}\underline{\smash{official site}}}\footnote{https://launchpad.net/gcc-arm-embedded}.
\\
...
\end{figure}
\\
A useful trick to note is that, within any file in windows, pressing the \textbf{\color{Green} SHIFT key} and \textbf{\color{Green} right clicking} inside the folder gives you the ability to open a command window (CMD) at the files current location. This isn't really important but it might come in handy.
\begin{figure}[hbtp]
\centering
...
\\
\-\ \hspace{20pt} \textbf{\color{Green} Control Panel $\rightarrow$ Advanced System Settings $\rightarrow$ Environment Variables}
\\
\centering
...
\-\ \hspace{20pt} \textbf{\color{Aquamarine} C:\textbackslash Program Files (x86)\textbackslash GNU Tools ARM Embedded\textbackslash 4.8 2014q2\textbackslash bin;}
\\
and then accept this by pressing \textbf{\color{Purple} OK} until you are out of the system properties tab. Depending on where you installed your Tool Chain to be, this location may change so find the \textbf{bin} folder within that location and add that value to the path variable
\end{minipage}
}
\justifying
...
\\
Editing the path variable, we will see there is already a value for arm-none-eabi's tools which was added in \ref{subsec:gccinst}, so after this we \underline{\smash{append}} or paste in the location of OpenOCD:
\\
...
\\
{\justifying \textbf{C:\textbackslash ~MinGW\textbackslash bin;C:\textbackslash ~Program Files ~(x86)\textbackslash ~GNU Tools ~ARM Embedded\textbackslash ~Q2\textbackslash ~bin;C:\textbackslash ~Program ~Files\textbackslash ~OpenOCD\textbackslash ~bin-x64;}}
\\
...
\\
Which opens the ST-Link Update window, first select \textbf{\color{Purple} "Device Connect"}. This will give you what appears to be an error about the device not being in DFU (\underline{\smash{Device Firmware Update}}) mode.\\
\begin{figure}[hbtp]
\centering
...
in procedure 'init'{
\\
\color{Black} So if you ever receive this sort of error, make sure all previous connections have been \textbf{\underline{\smash{terminated}}}.
\newpage
\justifying
...
So, first things first, we need to set up Java. Even though we will be using C/C++ to program our micro, Eclipse is an inherent Java IDE so download the latest {\color{Blue}\underline{\smash{\href{https://www.java.com/en/download/manual.jsp}{Java Runtime Edition}\footnote{https://www.java.com/en/download/manual.jsp}}}} (JRE) from the official Java website. Make sure to download the correct 64-bit or 32-bit version (depending on your operating system). 
...
You want to download the latest {\color{Blue}\underline{\smash{\href{http://https://eclipse.org/cdt/downloads.php}{Eclipse C development tool package}\footnote{https://eclipse.org/cdt/downloads.php}}}} (Eclipse CDT) at the time of writing; the latest build of Eclipse is Mars 8.8.1.\\
...
\begin{minipage}{0.9\textwidth}
\color{Red}
Seeing as some of you might be doing computer programming and you use Eclipse for Java development, rename this Eclipse folder to \textbf{\color{Green} Eclipse-arm-dev}, to differentiate it from your Java IDE.
\end{minipage}
}
\newpage
...
\newpage
\justifying
This is where you would install/update your CDT package if you needed to, expand the \textbf{\color{Purple} "CDT Optional Features"} and select both \underline{\smash{C/C++ GDB Hardware Debugging}} and \underline{\smash{C/C++ GCC Cross}} \underline{\smash{Compiler Support}} then click next and wait while it calculates the items which are to be installed.
\\
\begin{figure}[htbp]
\centering
...
\par
Once again navigate to the install new software window and add another repository, call it \textbf{\color{Purple} GNU ARM Eclipse plug-ins} and use the location: \textbf{\color{Orange} \underline{\smash{\href{http://gnuarmeclipse.sourceforge.net/updates}{http://gnuarmeclipse.sourceforge.net/updates}}}}.  Then select all the packages and install them. This will take quite a while to download/install so take this time to think about life and reflect on the decisions you've made which have led you here to Mechatronics, or think about some of the silly things you've said or done which you regret. If you feel inclined, you can read up on the \underline{\color{Blue} \smash{\href{http://gnuarmeclipse.livius.net/blog/}{Eclipse arm tools}}}\footnote{http://gnuarmeclipse.livius.net/blog/}.  
\newpage
\fbox{
\begin{minipage}{0.9\textwidth}
\color{Red}
